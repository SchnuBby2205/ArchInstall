...
function installDE() {
  ...
  if [[ "$desktop" == "meins" ]]; then
    ...
    runcmds 0 "Installing" "Programs..." "sudo pacman --noconfirm -S --needed firefox kitty dolphin ark unzip neovim fzf zsh lutris steam teamspeak3 lazygit git ${debugstring}"
    if [[ "$debug" =~ ^[nN]$ ]]; then printStepOK 1; fi
...
runcmds 0 "Downloading" "myShell..." "git clone --depth 1 https://github.com/SchnuBby2205/myShell.git ~/.config/quickshell/myShell ${debugstring}"
    ...
    runcmds 0 "Starting" "swww-daemon..." "echo -e 'exec-once=swww-daemon' >> $HOME/.config/hypr/schnubby/userprefs.conf"
    runcmds 0 "Setting" "myShell..." "echo -e 'exec-once=quickshell --path $HOME/.config/quickshell/myShell/shell.qml' >> $HOME/.config/hypr/schnubby/userprefs.conf"
