...
myPasswd() { a=0
  while [ $a -lt 3 ]; do
    ...
    (echo -e "$p1\n$p2" | sudo passwd "$1" &>/dev/null && myPrint print yellow "\nPassword updated succesfully.\n" && exit || exitWithError "Error setting the password.")
    ((a++))
  done
...
installDE() { checkDebugFlag
  ...
  [[ "$debug" =~ ^[nN]$ ]] && myPrint step ok
  git clone https://aur.archlinux.org/yay.git $debugstring; cd yay; makepkg -si; cd ..; rm -rf ./yay; printf "\n"
