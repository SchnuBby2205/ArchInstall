...
getInput() {
  ...
  read -r i
  printf -v "$v" "%s" "${i:-$d}"
  [[ -z "${!v}" ]] && exitWithError "Input value can not be empty!"
}
myPasswd() {
  a=0
  while [ $a -lt 3 ]; do
...
checkPartitions() {
  [[ -z "$boot" ]] && getInput "Enter boot partition: " boot
  [[ -z "$swap" ]] && getInput "Enter swap partition: " swap
  [[ -z "$root" ]] && getInput "Enter root partition: " root
}
checkDebugFlag() { debugstring=$([[ "$debug" =~ ^[yY]$ ]] && echo "" || echo " &>/dev/null"); }
runCMDS() {
  local s=$1 m=$2 msg=$3 cur=$4 fin=$5 max=$6
  shift 6
  [[ "$debug" =~ ^[nN]$ ]] && {
    ((cur > 0)) && printf "${CLEAR}${UP}"
    printf "["
    for ((i = 0; i < cur; i++)); do printf "#"; done
    for ((i = cur; i < max; i++)); do printf " "; done
    printf "]\n$m ${WHITE}$msg${NC}"
  }
  for c in "$@"; do $([ "$s" = 1 ] && echo sudo) bash -c "$c" || exitWithError "Command failed: $c"; done
  [[ "$debug" =~ ^[nN]$ ]] && {
    printf "${UP}\r["
    for ((i = 0; i < fin; i++)); do printf "#"; done
    for ((i = fin; i < max; i++)); do printf " "; done
    printf "]\n"
  }
}
installBaseSystem() {
  ...
  getInput "Type YES to continue (STRG+C to exit now)..." check "N"
  [[ "$check" != "YES" ]] && exitWithError "Formatting was not confirmed!" || printf "\n"
  ...
  umount -R /mnt $debugstring
  printf "\n"
  myPrint countdown 3 "Installation complete! Reboot in"
  reboot
}
installArchCHRoot() {
  checkDebugFlag
  [[ "$debug" =~ ^[nN]$ ]] && myPrint step Configuring "arch-chroot..."
  runCMDS 0 Setting localtime... 0 7 20 "ln -sf /usr/share/zoneinfo/${timezone} /etc/localtime $debugstring" "hwclock --systohc $debugstring"
  runCMDS 0 "Setting up" locales... 7 14 20 "sed -e '/${locale}/s/^#*//' -i /etc/locale.gen" "locale-gen $debugstring" "echo LANG=${locale} >> /etc/locale.conf" "echo KEYMAP=${keymap} >> /etc/vconsole.conf"
  ...
  [[ "$debug" =~ ^[nN]$ ]] && myPrint step ok
  [[ -z "$hostname" ]] && getInput "\nEnter your Hostname: " hostname "SchnuBbyLinux"
  echo ${hostname} >>/etc/hostname
  myPrint print yellow "\nEnter your NEW root password\n\n"
  myPasswd root
  [[ -z "$user" ]] && getInput "\nEnter your normal username: " user "schnubby"
  useradd -mG wheel ${user}
  myPrint print yellow "\Enter your normal user password\n\n"
  myPasswd "${user}"
  sed -e "/%wheel ALL=(ALL:ALL) ALL/s/^#*//" -i /etc/sudoers
  mv ./${scriptname} /home/${user}/
  echo ./${scriptname} installDE >>/home/${user}/.bashrc
}
installDE() {
  checkDebugFlag
  ...
  runCMDS 0 Installing "System dependencies..." 0 5 20 "sudo pacman --noconfirm -S --needed hyprland xdg-desktop-portal-{hyprland,gtk} sddm swww polkit-gnome xdg-user-dirs networkmanager $debugstring"
  runCMDS 0 Installing "Audio dependencies..." 5 10 20 "sudo pacman --noconfirm -S --needed pipewire{,-alsa,-audio,-pulse} gst-plugin-pipewire wireplumber pavucontrol pamixer $debugstring"
  runCMDS 0 Installing "Additional programs..." 10 19 20 "sudo pacman --noconfirm -S --needed firefox kitty dolphin ark unzip neovim fzf zsh lutris steam teamspeak3 lazygit git $debugstring"
  runCMDS 0 Installing Fonts... 19 20 20 "sudo pacman --noconfirm -S --needed ttf-jetbrains-mono-nerd $debugstring"
  [[ "$debug" =~ ^[nN]$ ]] && myPrint step ok
  git clone https://aur.archlinux.org/yay.git $debugstring
  cd yay
  makepkg -si
  cd ..
  rm -rf ./yay
  ...
  echo exec-once=kitty ./${scriptname} installConfigs >>$HOME/.config/hypr/schnubby/userprefs.conf
  myPrint countdown 3 "Reboot in"
  reboot
}
installConfigs() {
  checkDebugFlag
  ...
  case $gpu in
  amd) runCMDS 0 Installing "AMD drivers..." 0 5 20 "sudo pacman --noconfirm -S --needed mesa{,-utils} lib32{-mesa,-vulkan-radeon,-vulkan-icd-loader} vulkan{-radeon,-icd-loader} libva{-mesa-driver,-utils} $debugstring" ;;
  nvidia) runCMDS 0 Installing "NVIDIA drivers..." 0 5 20 "sudo pacman --noconfirm -S --needed nvidia{-dkms,-utils,-settings} lib32{-nvidia-utils, -vulkan-icd-loader} vulkan-icd-loader $debugstring" ;;
...
installSchnuBby() {
  checkDebugFlag
  [[ "$debug" =~ ^[nN]$ ]] && myPrint step Installing "SchnuBby specifics..."
  sudo mount --mkdir /dev/nvme0n1p4 /programmieren $debugstring
  for s in fstab autologin lutris zshhist gitconfig gitcred teamspeak3 grub firefox; do case $s in
    ...
    lutris)
      [[ -d "$HOME/.local/share/lutris" ]] && runCMDS 0 Backing lutris... 5 6 20 "mv $HOME/.local/share/lutris $HOME/.local/share/lutris_bak"
      [[ ! -d "$HOME/.local/share/lutris" ]] && runCMDS 0 Configuring lutris... 6 7 20 "ln -s /programmieren/backups/.local/share/lutris $HOME/.local/share/lutris"
      ;;
    zshhist)
      [[ -f "$HOME/.zsh_history" ]] && runCMDS 0 Removing .zsh_history... 7 8 20 "rm -rf $HOME/.zsh_history"
      runCMDS 0 Configuring .zsh_history... 7 9 20 "ln -sf /programmieren/backups/.zsh_history $HOME/.zsh_history"
      ;;
    ...
    teamspeak3)
      [[ -f "$HOME/.ts3client" ]] && runCMDS 0 Removing .ts3client... 13 14 20 "rm -rf $HOME/.ts3client"
    ...
    grub)
      ...
      ;;
    firefox)
      ff=$HOME/.mozilla/firefox/$(ls $HOME/.mozilla/firefox | grep "Default User")
      rm -rf "$ff"
      ln -sf /programmieren/backups/FireFox/3665cjzf.default-release "$ff"
      ;;
